...
class MakeBdvTestMixin(ABC):
    ...
    def _test_simple(self, shape, affine=None):
        ...
        make_bdv_from_dask_array(data, self.out_path, affine=affine)

        key = get_key(self.is_h5, timepoint=0, setup_id=0, scale=0)
        self.assertTrue(os.path.exists(self.out_path))
        with zarr.open(self.out_path, 'r') as f:
    ...
    def test_multi_setup(self):
        ...
        for vid in range(n_views):
            ...
            make_bdv_from_dask_array(data, self.out_path, setup_id=vid, affine=affine)
            data_dict[vid] = data
            affine_dict[vid] = affine

        # check implicit setup id
        data = da.random.random(shape).astype('float32')
        make_bdv_from_dask_array(data, self.out_path)
    ...
    def test_multi_timepoint(self):
        ...
        for tp in range(n_timepoints):
            data = da.random.random(shape)
            # make sure that we at least have 2 setup ids that agree
            setup_id = np.random.randint(0, 20) if tp > 1 else 0
            make_bdv_from_dask_array(data, self.out_path, setup_id=setup_id, timepoint=tp)
            tp_data.append(data)
    ...
    def test_custom_chunks(self):
        ...
        data = da.random.random(shape)
        make_bdv_from_dask_array(data, self.out_path, chunks=chunks)
    ...
    def test_custom_attributes(self):
        ...
        # write setup 0
        make_bdv_from_dask_array(data, self.out_path, setup_id=0,
        ...
        # write setup 1
        make_bdv_from_dask_array(data, self.out_path, setup_id=None,
        ...
        make_bdv_from_dask_array(data, self.out_path, setup_id=0, timepoint=1,
                 attributes={'channel': {'id': None},
                             'tile': {'id': 2},
                             'angle': {'id': 0}})
        attrs_out = get_attributes(self.xml_path, 0)
        ...
        # -> should fail
        with self.assertRaises(ValueError):
            make_bdv_from_dask_array(data, self.out_path, setup_id=0, timepoint=2,
